...
LEPTON_WORKSPACE_ID="${LEPTON_WORKSPACE_ID:-}"
...
LEPTON_LUSTRE_STORAGE="${LEPTON_LUSTRE_STORAGE:-lepton-shared-fs}"
...
case "$SOURCE" in
    physicsnemo-serve)
        ...
        : "${LEPTON_ENDPOINT_NAME:=${USER}-pnserve-ep}"
        ;;
    earth2studio)
        ...
        : "${LEPTON_ENDPOINT_NAME:=${USER}-earth2studio-ep}"
        ;;
    custom)
        : "${LEPTON_ENDPOINT_NAME:=${USER}-custom-ep}"
...
if [[ "$WAIT_FOR_READY" -eq 1 && "$DRY_RUN" -eq 0 ]]; then
    ...
    while [[ $(date +%s) -lt $end_time ]]; do
        ...
        summary="$(printf '%s\n' "$status_out" | grep -iE '^[[:space:]]*(state|status|endpoint)' | head -3 | tr -d '\r')"
        if [[ -n "$summary" && "$summary" != "$last_summary" ]]; then
            printf '%s\n' "$summary" | sed 's/^/    /'
            last_summary="$summary"
        fi
        if printf '%s' "$status_out" | grep -qiE '\b(Ready|Running)\b'; then
            break
        fi
        sleep 15
    done
...
if [[ "$DRY_RUN" -eq 0 ]]; then
    ...
    URL="$(lep endpoint status -n "$LEPTON_ENDPOINT_NAME" 2>&1 | grep -iE '^[[:space:]]*Endpoint[[:space:]]*[:=]' | head -1 | sed -E 's/^[[:space:]]*Endpoint[[:space:]]*[:=][[:space:]]*//' | tr -d '\r')"
    [[ -z "$URL" ]] && URL="(check status output above for the URL)"
else
