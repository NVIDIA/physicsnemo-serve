...
class FoundryFCN3Workflow(Earth2Workflow):
    ...
    cache_scope = "process"
    model_cache_names = ["FCN3"]
    cache_preserve_attributes = ("fcn3", "data")
    ...
    def __init__(
        ...
        # phases do not load GPU resources. The original Earth2Studio workflow
        # initializes these three fields here.
        self.fcn3: FCN3 | None = None
    ...
    def _validate_plugin_parameters(
        ...
        if not 1 <= n_samples <= _MAX_ENSEMBLE_SAMPLES:
            raise ValueError(
    ...
    def validate_variables(self, variables: Sequence[str] | None) -> np.ndarray:
        ...
        if self.fcn3 is None:
            raise RuntimeError("FCN3 model must be loaded before validating variables")
    ...
    def __call__(
        self,
        io: IOBackend,
        start_time: datetime | str = datetime(2025, 1, 1),
        n_steps: int = 12,
        n_samples: int = 4,
        ...
        start_time = self._coerce_start_time(start_time)
    ...
    def cleanup(self) -> None:
        try:
            ...
            self._release_models(self.fcn3)
            self._clear_attributes("fcn3", "data")
            self._cleanup_torch_runtime(device=str(self.device))


WORKFLOW = FoundryFCN3Workflow
